...
#define CHECK_VK(p, m) ERROR_IF((p) != VK_SUCCESS, "createVulkan()", (m), deleteVulkanApp(app), NULL)
#define CHECK(p, m)    ERROR_IF(!(p),              "createVulkan()", (m), deleteVulkanApp(app), NULL)
...
#define FIRST_QUERY_INDEX 0
#define QUERIES_COUNT 2
#define ENTITIES_COUNT 100

    // reset a query pool
    vkResetQueryPool(app->device, app->queryPool, FIRST_QUERY_INDEX, QUERIES_COUNT);
    ...
    // write a timestamp
    vkCmdWriteTimestamp(cmdBuffer,VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, app->queryPool, FIRST_QUERY_INDEX);

    // start a render pass
    {
#define CLEAR_VALUES_COUNT 1
        const VkClearValue clearValues[CLEAR_VALUES_COUNT] = {
            {{ 0.5f, 0.0f, 0.0f, 1.0f }},
        };
...
#undef CLEAR_VALUES_COUNT
    ...
    // write a timestamp
    vkCmdWriteTimestamp(cmdBuffer, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, app->queryPool, FIRST_QUERY_INDEX + 1);
    ...
    // wait for device
    CHECK_VK(vkDeviceWaitIdle(app->device), "failed to wait the device");

    return 1;

#undef QUERIES_COUNT
#undef FIRST_QUERY_INDEX
#undef CHECK
#undef CHECK_VK
}
